...
int main(int argc, char *argv[])
{
    ...
    while(IS_WORKING){
        ...
        {
            char imu_cmd_type = sharedCMD->COMMAND[RBCORE_PODO_NO].USER_PARA_CHAR[0];
            int IMUch = 1;
            if(imu_cmd_type == 1){
                RBCAN_MB mb;
                mb.channel = IMUch;
...
//                if(fabs(sharedSEN->ENCODER[i][j].CurrentReference-finalJointRef[j]) < 1)

                  sharedSEN->ENCODER[i][j].CurrentReference = finalJointRef[j];
//                else if(i==4 || (i==10&&j==1) || (i==21&&j==1) || (i==22&&j==1))
//                    sharedSEN->ENCODER[i][j].CurrentReference = finalJointRef[j];
//                else
//                    ;
//                    printf("%d,%d joint jump!!!!\n",i,j);
...
//        printf("%d | %f     walkready = %f        roscomand = %f\n",sharedCMD->MotionOwner[12][0],sharedSEN->ENCODER[12][0].CurrentReference,sharedREF->JointReference[2][12][0],sharedREF->JointReference[6][12][0]);
...
void THREAD_ReadEncoder(){
    ...
    for(int i=0; i<_NO_OF_MC; i++){
        ...
        if(i==(MC_GetID(RHAND)) || i==(MC_GetID(LHAND)))
            _DEV_MC[i].RBBoard_ReadEncoderDataGRIPPER();
        else
